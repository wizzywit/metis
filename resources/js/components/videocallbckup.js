...
let socketUrl = `wss://${location.host}/ws`;
// const socketUrl = `ws://localhost:3000/ws`;
let socket = new WebSocket(socketUrl);
let ringer = new Audio("/videocall/Assets/audio/ringtone.mp3");
ringer.loop = true;
var interval;
...
class VideoCall extends Component {
    ...
    async handleAcceptIncommingCall() {
        ...
        this.setState({ call }, () => {
            clearInterval(interval);
            webrtc.setRemoteDescription(this.state.call.message.answer);
            this.sendMessageToSignallingServer({
                channel: "call_accepted",
                otherPerson: this.state.otherPerson
            });
            this.updateUserAccepted(this.state.call.message);
        });
    }
